...
def calculateLigandReceptorIntegrationScore(
    ...
    cellxcell = kneighbors_graph(X_spatial, n_neighbors=number_nearest_neighbors, mode='distance', include_self=True)
...
def runZoneTalk(
    ...
    # Use the modified QC metrics calculation for small gene counts
        sc.pp.calculate_qc_metrics(lr_adata, inplace=True, percent_top=[50, 100])
    else:
    # Use the standard QC metrics calculation
